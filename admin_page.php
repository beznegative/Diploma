...
<head>
   ...
   <title>Показатели | Кафе «Олимпия»</title>
...
<section class="dashboard">
   ...
   <div class="box-container">
      ...
      <div class="box">
         <?php
            $total_pendings = 0;
            $select_pendings = mysqli_query($conn, "SELECT * FROM `orders` WHERE payment_status = 'В ожидании'") or die('query failed');
            while($fetch_pendings = mysqli_fetch_assoc($select_pendings)){
               $total_pendings += $fetch_pendings['total_price'];
            };
         ?>
         <h3><?php echo $total_pendings; ?> р.</h3>
         <p>Сумма не завершенных заказов</p>
      </div>

      <div class="box">
         <?php
            $total_completes = 0;
            $select_completes = mysqli_query($conn, "SELECT * FROM `orders` WHERE payment_status = 'Завершенный'") or die('query failed');
            while($fetch_completes = mysqli_fetch_assoc($select_completes)){
               $total_completes += $fetch_completes['total_price'];
            };
         ?>
         <h3><?php echo $total_completes; ?> р.</h3>
         <p>Сумма завершенных заказов</p>
      </div>
